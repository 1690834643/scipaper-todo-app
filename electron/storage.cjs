...
// storage.cjs is the public facade. Tests and MCP tools may reload only this
// module after changing HOME/USERPROFILE, so refresh split internals here to
// preserve the old single-file behavior where paths were recomputed per load.
for (const modulePath of ['./schema.cjs', './normalize.cjs', './db.cjs']) {
...
const { ensureStore, readDatabase, writeDatabase } = require('./db.cjs');
const {
  now,
  createId,
  normalizeText,
  ...
  for (const section of [...thesis.sections].sort((left, right) => left.orderIndex - right.orderIndex)) {
    ...
    for (const block of blocks) {
      if (normalizeBlockType(block.type) === 'Text') {
        // Markdown export \u2014 render plain prose, not the editor's HTML markup.
  ...
  articles.forEach(article => {
    article.sections.forEach(section => {
      section.contentBlocks.forEach(block => {
        if (block.type === 'Text') {
          // Word frequency stats need plain prose, not the HTML markup.
...
function countWords(text) {
  ...
  // block.content is HTML; strip tags before counting so attributes/tag names
